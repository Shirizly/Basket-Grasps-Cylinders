function [flag,eigv] = MasonConditionFunction(r,hcm,t,ph,ds)
%% Basic setup
flag = 0;
% syms t ph hcm r ds
...
%% condition - CONSTRUCT WITH TWO GENERATORS PER MY WORK, COMPARE TO RESULTS FROM MASON!!!
u1 = [cross(p2,l1);l1]; 
u2 = [cross(p1,l2);l2];
u3 = [cross(p1,l3);l3];
...
G = Gpart2 + Gpart1;
eigv = eig(G);
if all(eigv<=0) && sum(eigv)<min(eigv)
    flag = 1;
end
end
