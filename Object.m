classdef Object
% A cylinder defined by a length and radius.
properties
    radi = []; % circular cross-section's radius
    length = []; % length along cylinder's axis
    com = []; % center of mass location (along the cylinder axis).
%     J = [0 -1;1 0]; % rotation matrix by 90 degrees CCW
    res = 5; % resolution of the boundary parametrization
    gv = [0;0;-1]; % gravity direction
    epsilon = 5e-3;
    S = [];
    X = [];
    VL = [];
...
methods
    ...
    function PG = Polygon(varargin)
            switch nargin
                ...
                case 2 % Input is: radius, length
                    PG.radi = varargin{1}; 
                    PG.length = varargin{2};
                    PG.com = 0.5*PG.length;
                case 3 
                    PG.radi = varargin{1}; 
                    PG.length = varargin{2};
                    PG.com = varargin{3};
            end
            PG.scale = sqrt(PG.length^2+4*PG.radi^2);
    end
